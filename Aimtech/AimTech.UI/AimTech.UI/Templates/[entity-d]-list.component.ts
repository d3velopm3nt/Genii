...
export class [Entity]ListComponent implements OnInit {
  ...
  edit([entity-cc]) {
    if ([entity-cc])
      this.service.[Entity-cc] = [entity-cc];
    this.router.navigate([AppRoutes.[entity-cc]Config]);
  }

  new[Entity]() {
    this.service.[Entity-cc] = undefined;
    this.router.navigate([AppRoutes.[entity-cc]Config]);
  }
  ...
  openDeleteDialog([entity-cc]){
    const dialogRef = this.dialog.open(RemoveDialogComponent);

    dialogRef.afterClosed().subscribe(result =>{
      if(result === 'Yes'){
        this.service.delete([entity-cc]);
